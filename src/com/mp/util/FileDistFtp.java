...
public class FileDistFtp {
    ...
    public void getFile(Hashtable inputhash) throws Err {
		...
		//입력 값 체크	
		SVRIP = (String)inputhash.get("SVRIP");
		if(SVRIP == null || SVRIP.equals("")) {		//서버 IP가 없다면
			errflag = true;
			errstr = " [서버 IP] ";
		}
		SVRPORT = (String)inputhash.get("SVRPORT");
		if(SVRPORT == null || SVRPORT.equals("")) {	//서버 PORT가 없다면	
			errflag = true;
			errstr = " [서버 PORT] ";
		}
		USER = (String)inputhash.get("USER");
		if(USER == null || USER.equals("")) {		//USER ID
			errflag = true;
			errstr = " [유저아이디] ";
		}
		PASSWORD = (String)inputhash.get("PASSWORD");
		if(PASSWORD == null || PASSWORD.equals("")) {	//USER PASSWORD
			errflag = true;
			errstr = " [패스워드] ";
		}
		SVRPATH = (String)inputhash.get("SVRPATH");
		if(SVRPATH == null || SVRPATH.equals("")) {	//서버전체경로
			errflag = true;
			errstr = " [수신지패스] ";
		}
		LOCALPATH = (String)inputhash.get("LOCALPATH");
		if(LOCALPATH == null || LOCALPATH.equals("")) {	//로컬전체경로
			errflag = true;
			errstr = " [전송할 파일 패스] ";
		}
		MODE = (String)inputhash.get("MODE");
		if(MODE == null || MODE.equals("")) {		//파일 생성 모드
			MODE = "C";
		}
     ...
     */
     public String getPlatformPath() {
		String PATH_GUBUN = null;
        if(getPlatform() == Code.UNIX) PATH_GUBUN = "/";
        else PATH_GUBUN = "\\";
	...
	public static void main(String[] args) throws Exception {
		...
		/* ====================================================
			...
			usage : java -Denvfile=[환경화일패스] com.mp.util.FileDistFtp [G(수신)/S(전송)]
	...
	**************************************************************/
		...
		if(args[0].equals("G")) {	
			inputhash.put("SVRIP","211.104.124.39");
			...
			filedistftp.getFile(inputhash);
		} else if(args[0].equals("S")) {
			inputhash.put("SVRIP","211.104.124.38");
			inputhash.put("SVRPORT","21");
